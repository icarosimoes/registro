...
def _tmpl_row_to_out(row, items=None) -> schemas.TemplateOut:
    ...
    if isinstance(row, dict):
        return schemas.TemplateOut(
            ...
            recurrence=tmpl.recurrence, category=tmpl.category,
            assigned_user_id=tmpl.assigned_user_id,
            assigned_user_name=row.get("assigned_user_name"),
            active=tmpl.active, next_due=tmpl.next_due,
...
def _exec_to_out(data, with_items=False) -> schemas.ExecutionOut:
    if isinstance(data, dict):
        ...
        return schemas.ExecutionOut(
            id=exc.id, template_id=exc.template_id,
            template_name=row.template_name,
            due_date=exc.due_date, status=exc.status,
            completed_at=exc.completed_at,
            completed_by_user_id=exc.completed_by_user_id,
            completed_by_name=row.completed_by_name,
            notes=exc.notes, progress=progress,
            items=[schemas.ExecutionItemOut(
                id=i.id, label=i.label, sort_order=i.sort_order,
                checked=i.checked, checked_at=i.checked_at,
            ) for i in items] if with_items else None,
            created_at=exc.created_at, updated_at=exc.updated_at,
        )
    exc = data[0]
    return schemas.ExecutionOut(
        id=exc.id, template_id=exc.template_id,
...
async def create_checklist_template(
    ...
    try:
        data = await create_template(
            ...
            recurrence=body.recurrence, category=body.category,
            assigned_user_id=body.assigned_user_id, next_due=body.next_due,
...
async def update_checklist_template(
    ...
    try:
        data = await update_template(
            session, user.company_id, user.id, template_id, updates,
        )
    except ValueError as exc:
        raise HTTPException(422, detail=str(exc)) from None
    if data is None:
...
async def delete_checklist_template(
    ...
    if not await delete_template(
        session, user.company_id, user.id, template_id,
    ):
        raise HTTPException(404, detail="Template não encontrado")
...
async def list_checklist_executions(
    ...
    session: Annotated[AsyncSession, Depends(require_session)],
    page: int = 1, page_size: int = 25,
    template_id: int | None = None, status: str | None = None,
):
    rows, total = await list_executions(
        session, user.company_id, page, page_size, template_id, status,
    )
...
async def toggle_checklist_item(
    ...
    data = await toggle_item(
        session, user.company_id, user.id,
        execution_id, body.item_id, body.checked,
    )
    if data is None:
...
async def complete_checklist_execution(
    ...
    data = await complete_execution(
        session, user.company_id, user.id, execution_id, body.notes,
    )
    if data is None:
        raise HTTPException(404, detail="Execução não encontrada")
    return _exec_to_out(data, with_items=True)
