...
def _mov_to_out(row) -> schemas.MovementOut:
    ...
    return schemas.MovementOut(
        id=mov.id, item_id=mov.item_id,
        item_name=row.item_name,
        movement_type=mov.movement_type, quantity=mov.quantity,
        reason=mov.reason, work_order_id=mov.work_order_id,
        occurrence_id=mov.occurrence_id, user_id=mov.user_id,
        user_name=row.user_name, created_at=mov.created_at,
    )
...
async def list_stock_items(
    ...
    search: str | None = None, below_min: bool = False,
):
    rows, total = await list_items(
        session, user.company_id, page, page_size, search, below_min,
    )
...
async def update_stock_item(
    ...
    row = await update_item(
        session, user.company_id, user.id, item_id, updates,
    )
    if row is None:
        raise HTTPException(404, detail="Item não encontrado")
    return _item_to_out(row)
...
async def delete_stock_item(
    ...
    if not await delete_item(
        session, user.company_id, user.id, item_id,
    ):
        raise HTTPException(404, detail="Item não encontrado")
...
async def create_stock_movement(
    ...
    return schemas.MovementOut(
        ...
        reason=mov.reason, work_order_id=mov.work_order_id,
        occurrence_id=mov.occurrence_id, user_id=mov.user_id,
        user_name=data["user_name"], created_at=mov.created_at,
    )
...
async def list_stock_movements(
    ...
    rows, total = await list_movements(
        session, user.company_id, page, page_size, item_id,
    )
