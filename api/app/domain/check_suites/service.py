...
async def list_check_suites(
    ...
    item_count = (
        ...
        .where(CheckSuiteItem.suite_id == CheckSuite.id)
        .correlate(CheckSuite)
        .scalar_subquery()
    )
    rows = (
        await session.execute(
            select(CheckSuite, User.name.label("owner_name"), item_count.label("item_count"))
            .outerjoin(User, User.id == CheckSuite.owner_user_id)
            .where(*filters)
            .order_by(CheckSuite.updated_at.desc(), CheckSuite.id.desc())
...
async def update_check_suite(
    ...
    await session.commit()
    await session.refresh(record)
    return await get_check_suite(session, company_id, record.id)
