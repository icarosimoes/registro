...
def upgrade() -> None:
    ...
    if has_hotel is not None:
        conn.execute(
            text("UPDATE users SET company_id = :target WHERE company_id = :source").bindparams(
                target=has_hotel, source=has_v1
            )
        )
        conn.execute(
            text(
        ...
        conn.execute(
            text("UPDATE sectors SET company_id = :target WHERE company_id = :source").bindparams(
                target=has_hotel, source=has_v1
            )
        )
        conn.execute(
            text("UPDATE locations SET company_id = :target WHERE company_id = :source").bindparams(
                target=has_hotel, source=has_v1
            )
        )
        conn.execute(
            text("UPDATE functions SET company_id = :target WHERE company_id = :source").bindparams(
                target=has_hotel, source=has_v1
            )
        )
        conn.execute(
