...
def upgrade() -> None:
    ...
    op.add_column(
        "fiscal_requests", sa.Column("requester_user_id", sa.Integer(), sa.ForeignKey("users.id"))
    )
    op.add_column(
        "fiscal_requests", sa.Column("responsible_user_id", sa.Integer(), sa.ForeignKey("users.id"))
    )
